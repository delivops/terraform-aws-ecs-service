...
module "api" {
  ...
  container_name  = "app"
  container_image = "my-repo:stable" # must exist: see below
...
| `otel_collector` | An `otel-collector` container on ports 4317 (gRPC) and 4318. With no `image_name` or `image`, it runs the public ADOT image with its config read from the SSM parameter `ssm_name`, so the **execution role** needs `ssm:GetParameters` on that parameter. |
...
| `container_overrides` | Container name ⇒ ECS API fields merged over that generated container (e.g. `ulimits`, `dockerLabels`). Every key must name a generated container. The merge is top-level only: a field such as `environment` or `mountPoints` replaces the generated list rather than adding to it. |

`image_name` on either collector is a repository in the ECR registry of the
cluster's account; `image` is a full image reference.

Plans fail on the mistakes that ECS would otherwise reject at registration, or
that would only surface on the next deploy or when a task starts:
...
- Volume names contain only letters, digits, hyphens and underscores, so a
  `writable_dirs` path can't contain characters such as dots.
- `app_protocol` is `http`, `http2`, `grpc` or `tcp`.
- Under `awsvpc` and `host`, no two containers use the same container port.
- The template maps every `container_port` the service's load balancers forward
  to. With Service Connect as `client-server`, it has the `default` port (from
  `port`) and each `service_connect.additional_ports` name as an
  `additional_ports` key, and `app_protocol = "tcp"` exactly when
  `service_connect.appProtocol` is `tcp`.
- `secrets` values and `secrets_envs` ids are full secret ARNs.
- `secret_files` require a task role.
- On Fargate, `cpu` and `memory` are a supported combination, the sidecars'
  `cpu` and `memory` each total less than the task's, and
  `ephemeral_storage_gib` is 21–200. Ephemeral storage is Fargate-only.
...
`container_image` does two jobs. In the template it is a placeholder the
pipeline replaces. It is also the image of the write-once task definition a
**new** service starts on, before its first deploy. So it must be an image the
service can pull, or the first tasks fail until the pipeline deploys. The
alternative is `desired_count = 0` until the first deploy sets the count.
Enabling the template on an existing service changes nothing until its next
deploy.
