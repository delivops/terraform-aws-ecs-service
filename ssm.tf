...
# /<cluster>/<service>/role — the task/execution role ARN.
...
resource "aws_ssm_parameter" "role" {
  ...
  name  = "/${var.ecs_cluster_name}/${var.ecs_service_name}/role"
  type  = "String"
  value = local.service_role_arn
  tags  = local.common_tags
}

# /<cluster>/<service>/tags — the effective tags ({ Application } merged with var.tags), JSON-encoded.
resource "aws_ssm_parameter" "tags" {
  name  = "/${var.ecs_cluster_name}/${var.ecs_service_name}/tags"
  type  = "String"
